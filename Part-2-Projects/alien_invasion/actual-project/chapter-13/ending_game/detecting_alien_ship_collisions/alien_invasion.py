...
class AlienInvasion:
    ...
    def _update_alien(self):
        ...
        if pygame.sprite.spritecollideany(self.ship, self.aliens):
            if self.settings.ship_limit == 0:
                self._print_game_over_message()
                sys.exit()

            if self.settings.ship_limit>=2:
                self.settings.ship_limit -=1
                self.bullets.empty()
                self.aliens.empty()
                self._print_new_round_message()
                self._create_alien_fleet()
                self.ship.blitme()
                sleep(0.5)
    ...
    def _print_game_over_message(self):
    ...
    def _print_new_round_message(self):
        font = pygame.font.Font(None, 48)
        text_surface = font.render("New Round is being started in a bit", True, (255,255,255))
        # center =(self.settings.screen_height/2, self.settings.screen_width/2)
